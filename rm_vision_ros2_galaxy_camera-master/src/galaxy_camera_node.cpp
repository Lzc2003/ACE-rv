...
public:
  explicit GalaxyCameraNode(const rclcpp::NodeOptions & options) : Node("galaxy_camera", options)  
  {
    ...
    while (true) {
      ...
      //status = GXOpenDeviceByIndex(2, &camera_handle_);
     //GX_OPEN_PARAM a;
     // a.openMode = GX_OPEN_SN;
     // a.accessMode = GX_ACCESS_EXCLUSIVE;
     // const char* name = "KE0200010100";
     // a.pszContent = const_cast<char*>(name);
     // status = GXOpenDevice(&a, &camera_handle_);
    ...
    GXSetInt(camera_handle_, GX_INT_WIDTH, 640);
    GXSetInt(camera_handle_, GX_INT_WIDTH_MAX, img_info_.nWidthMax);
    GXSetInt(camera_handle_, GX_INT_HEIGHT, 480);
    GXSetInt(camera_handle_, GX_INT_HEIGHT_MAX, img_info_.nHeightMax);
